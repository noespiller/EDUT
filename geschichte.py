...
def erstes_kapitel():
    ...
    \nund versuchst zu erspähen wer oder was so dringend nach deiner Aufmerksamkeit verlangt.
    ...
    \nObwohl deine Augen klare Strukturen erkennen, kannst du niemand entdecken.
    \njedoch fällt dir jetzt auf wie dunkel es hier ist.
    \n"Hatte ich nicht auf einer Waldlichtung geschlafen?"
    \nDu spürst einen kurzen aber doch sehr schmerzhaften Schlag gegen deinen Fuß.
    ...
    code = layouts.aufgabe_layout(title, 'Stelle dich vor, indem du deinen Namen auf der Konsole ausgibst. (Wenn du nicht weiterweißt, schaue das Video über die print() Funktion und mache ein paar Aufgaben)')
...
def zweites_kapitel(name):
    ...
    code = layouts.aufgabe_layout(title, 'Speicher deinen Namen, Beruf und Alter als String in der Variablen "blatt".')
    
    def evaluate(co, out):
        return "blatt=" in co.split("\n")[-1] or "blatt =" in co.split("\n")[-1]
